...
async def get_actor_detail(actor_id: int, db: AsyncSession = Depends(get_db),
    ...
    return {
        ...
        "wallets": [{"id": w.id, "address": w.address, "currency": w.currency,
                     "balance": w.balance, "transaction_count": w.transaction_count} for w in wallets],
        "domains": [{"id": d.id, "domain": d.domain, "is_tor": d.is_tor} for d in domains],
        "ips": [{"id": i.id, "ip_address": i.ip_address, "asn": i.asn,
                 "country": i.country, "is_tor": i.is_tor} for i in ips],
